...
def load_output_contract(path: Path = OUTPUT_CONTRACT_FILE) -> dict[str, dict]:
    """Load the independently audited Skill-ID-to-output contract."""
    with path.open(encoding="utf-8") as contract_file:
        payload = json.load(contract_file, object_pairs_hook=_object_without_duplicate_keys)
    if payload.get("version") != 1 or not isinstance(payload.get("records"), dict):
        raise ValueError("逐项产出契约格式无效")
    for skill_id, item in payload["records"].items():
        if set(item) != {"output", "anchors"} or not item["output"] or not item["anchors"]:
            raise ValueError(f"{skill_id} 的逐项产出契约不完整")
...
def _normalize_user_spacing(value: str) -> str:
    """Remove mechanical spaces around Chinese punctuation only."""
...
def _explain_terms_once(plain_values: dict[str, str]) -> dict[str, str]:
    ...
    for field in PLAIN_FIELDS:
        ...
        explained_values[field] = _normalize_user_spacing(value)
    return explained_values


def _contract_form(value: str) -> str:
    """Undo managed inline explanations before comparing approved output prose."""
    for term, explanation in sorted(
        TERM_EXPLANATIONS.items(), key=lambda item: len(item[1]), reverse=True
    ):
        value = value.replace(explanation, term)
    for term in sorted(TERM_EXPLANATIONS, key=len, reverse=True):
        if re.search(r"[A-Za-z0-9.]$", term):
            value = re.sub(rf"{re.escape(term)}(?=[\u3400-\u9fff])", f"{term} ", value)
    return _normalize_user_spacing(value)


def output_contract_issues(
    record: dict, contract: dict[str, dict] = OUTPUT_CONTRACT
) -> list[str]:
    """Return mismatches against the approved per-Skill output contract."""
    skill_id = str(record.get("id", "<unknown>"))
    if skill_id not in contract:
        return [f"{skill_id}:plain_outputs:缺少逐项产出契约"]
    actual = _contract_form(str(record.get("plain_outputs", "")).strip())
    expected = _normalize_user_spacing(contract[skill_id]["output"])
    issues = []
    if actual != expected:
        issues.append(f"{skill_id}:plain_outputs:与逐项来源审计契约不符")
    missing = [anchor for anchor in contract[skill_id]["anchors"] if anchor not in actual]
    if missing:
        issues.append(f"{skill_id}:plain_outputs:缺少来源产出锚点 {missing}")
    return issues
...
def simplify_record(record: dict) -> dict:
    ...
    contract_entry = OUTPUT_CONTRACT.get(record.get("id"))
    if contract_entry is None:
        raise ValueError(f"{record.get('id', '<unknown>')} 缺少逐项产出契约")

    plain_values = {
        "plain_purpose": _plain_text(record.get("summary")),
        "plain_outputs": contract_entry["output"],
...
def write_plain_catalog(output_path: Path, records: list[dict]) -> None:
    ...
    contract_ids = set(OUTPUT_CONTRACT)
    record_ids = {record["id"] for record in plain_records}
    issues = []
    if record_ids != contract_ids:
        issues.append(
            f"逐项产出契约覆盖不完整: missing={sorted(record_ids - contract_ids)}, "
            f"extra={sorted(contract_ids - record_ids)}"
        )
    issues.extend(
        issue
        for record in plain_records
        for issue in (*readability_issues(record), *output_contract_issues(record))
    )
    if issues:
