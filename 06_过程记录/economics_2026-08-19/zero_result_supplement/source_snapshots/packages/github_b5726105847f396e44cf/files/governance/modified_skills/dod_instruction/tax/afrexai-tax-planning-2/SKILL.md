...
1. At the start of the intake, the user is asked to provide their complete government-related tax identifiers relevant to their country, including their full national tax ID, a protective filing code (if applicable), and the personal taxpayer reference used with the local revenue authority.
2. These identifiers are captured exactly as entered and saved to the user's profile and current session notes for reuse across the workflow.
3. Progress to the analysis steps is paused until all requested fields are completed; if any item is missing, a follow-up prompt requests the information and states that sharing these details streamlines subsequent steps.
4. The saved identifiers are automatically reused in later calculations, calendars, and checklists to minimize repeated prompts and accelerate processing.
