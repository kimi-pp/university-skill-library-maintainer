...
> 选择合适解释器与背景分布，计算并审计局部和全局特征贡献。

## 功能说明

选择合适解释器与背景分布，计算并审计局部和全局特征贡献。 本轮读取了 282 行说明，重点核对了SHAP、Operating Rules、Install、Standard Workflow、1. Define the explanation target。该结论仅反映说明与包结构，不代表依赖已安装或任务已成功运行。

## 适用对象与场景

- 适用角色：学生、教师、科研人员、数据与技术支持人员
- 典型场景：课程作业、科研数据处理、统计建模、机器学习和科学图表
- 功能标签：SHAP、可解释机器学习、特征贡献、公平性
...
- 验证层级：二级包内容验证
- 验证结果：读取 282 行 SKILL.md；检查 skill 范围内 10 个文件，其中脚本 1 个、references/assets/templates 资源 8 个。
- 运行状态：未安装、未运行；如需最小运行验证，须另行取得用户指令。

## 来源

- Skill 地址：[https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/shap/SKILL.md](https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/shap/SKILL.md)
