...
# Zarr 分块数组存储（zarr-python）

> 创建、读写和组织带分块与压缩的 N 维数组，支持本地或对象存储工作流。

## 功能说明

创建、读写和组织带分块与压缩的 N 维数组，支持本地或对象存储工作流。 本轮读取了 241 行说明，重点核对了Zarr Python、Overview、Quick Start、Installation、Basic Array Creation。该结论仅反映说明与包结构，不代表依赖已安装或任务已成功运行。

## 适用对象与场景

- 适用角色：学生、教师、科研人员、数据与技术支持人员
- 典型场景：课程作业、科研数据处理、统计建模、机器学习和科学图表
- 功能标签：Zarr、分块、压缩、云存储、数组
...
- 验证层级：二级包内容验证
- 验证结果：读取 241 行 SKILL.md；检查 skill 范围内 7 个文件，其中脚本 0 个、references/assets/templates 资源 6 个。
- 运行状态：未安装、未运行；如需最小运行验证，须另行取得用户指令。

## 来源

- Skill 地址：[https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/zarr-python/SKILL.md](https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/zarr-python/SKILL.md)
