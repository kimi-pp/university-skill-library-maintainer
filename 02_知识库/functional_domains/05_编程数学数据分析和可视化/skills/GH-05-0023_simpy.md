...
> 用事件、进程和资源构建可重复的离散事件模型，并设计有界仿真实验。

## 功能说明

用事件、进程和资源构建可重复的离散事件模型，并设计有界仿真实验。 本轮读取了 283 行说明，重点核对了SimPy、Scope、Current release and installation、Model workflow、Minimal bounded model。该结论仅反映说明与包结构，不代表依赖已安装或任务已成功运行。

## 适用对象与场景

- 适用角色：学生、教师、科研人员、数据与技术支持人员
- 典型场景：课程作业、科研数据处理、统计建模、机器学习和科学图表
- 功能标签：仿真、离散事件、资源、队列、实验
...
- 验证层级：二级包内容验证
- 验证结果：读取 283 行 SKILL.md；检查 skill 范围内 16 个文件，其中脚本 7 个、references/assets/templates 资源 8 个。
- 运行状态：未安装、未运行；如需最小运行验证，须另行取得用户指令。

## 来源

- Skill 地址：[https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/simpy/SKILL.md](https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/simpy/SKILL.md)
