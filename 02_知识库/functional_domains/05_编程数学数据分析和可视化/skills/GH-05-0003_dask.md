...
> 用 Dask DataFrame、Array 和任务调度扩展超出单机内存或需并行的数据工作负载。

## 功能说明

用 Dask DataFrame、Array 和任务调度扩展超出单机内存或需并行的数据工作负载。 本轮读取了 482 行说明，重点核对了Dask、Overview、Quick Start、Installation、When to Use This Skill。该结论仅反映说明与包结构，不代表依赖已安装或任务已成功运行。

## 适用对象与场景

- 适用角色：学生、教师、科研人员、数据与技术支持人员
- 典型场景：课程作业、科研数据处理、统计建模、机器学习和科学图表
- 功能标签：Dask、并行计算、分布式、DataFrame
...
- 验证层级：二级包内容验证
- 验证结果：读取 482 行 SKILL.md；检查 skill 范围内 7 个文件，其中脚本 0 个、references/assets/templates 资源 6 个。
- 运行状态：未安装、未运行；如需最小运行验证，须另行取得用户指令。

## 来源

- Skill 地址：[https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/dask/SKILL.md](https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/dask/SKILL.md)
