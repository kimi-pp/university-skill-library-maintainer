...
> 按标准贝叶斯工作流选择先验与似然、采样、诊断和解释后验不确定性。

## 功能说明

按标准贝叶斯工作流选择先验与似然、采样、诊断和解释后验不确定性。 本轮读取了 310 行说明，重点核对了PyMC Bayesian Modeling、Overview、Current Version and Setup、When to Use This Skill、Standard Bayesian Workflow。该结论仅反映说明与包结构，不代表依赖已安装或任务已成功运行。

## 适用对象与场景

- 适用角色：学生、教师、科研人员、数据与技术支持人员
- 典型场景：课程作业、科研数据处理、统计建模、机器学习和科学图表
- 功能标签：贝叶斯、MCMC、先验、后验、诊断
...
- 验证层级：二级包内容验证
- 验证结果：读取 310 行 SKILL.md；检查 skill 范围内 10 个文件，其中脚本 2 个、references/assets/templates 资源 7 个。
- 运行状态：未安装、未运行；如需最小运行验证，须另行取得用户指令。

## 来源

- Skill 地址：[https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/pymc/SKILL.md](https://github.com/K-Dense-AI/scientific-agent-skills/blob/main/skills/pymc/SKILL.md)
