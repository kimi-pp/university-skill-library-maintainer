...
class ThesisReviewer:
    ...
    def generate_review_document(self, output_path):
        """Generate review document in Word format."""
        doc = Document()

        # Title
        title = doc.add_heading('硕士学位论文评审意见', 0)
        title.alignment = WD_ALIGN_PARAGRAPH.CENTER

        # Metadata
        doc.add_paragraph(f'评审文件: {os.path.basename(self.input_path)}')
        doc.add_paragraph(f'生成日期: {datetime.now().strftime("%Y年%m月%d日")}')
        doc.add_paragraph(f'评审人: Claude Code 学术评审系统')
        doc.add_paragraph('')

        # 1. Overall assessment
        doc.add_heading('1. 总体评价', 1)
        ...
        overall.add_run('本文基本符合硕士学位论文要求，但在格式规范、学术表达和结构组织方面存在一些需要改进的问题。')
        overall.add_run(f'共发现{len(self.issues)}个问题，其中：')

        doc.add_paragraph(f'• 高优先级问题: {len(high_issues)}个')
        doc.add_paragraph(f'• 中优先级问题: {len(medium_issues)}个')
        doc.add_paragraph(f'• 低优先级问题: {len(low_issues)}个')
        doc.add_paragraph('')

        # 2. Detailed findings
        doc.add_heading('2. 具体问题及修改建议', 1)
        ...
        for eng_cat, chi_cat in categories.items():
            ...
            if cat_issues:
                doc.add_heading(f'2.{list(categories.keys()).index(eng_cat)+1} {chi_cat}', 2)
                ...
                for issue in cat_issues:
                    ...
                    issue_text = doc.add_paragraph(style='List Bullet')
                    issue_text.add_run(f'{priority_indicator} {issue["description"]} ')
                    issue_text.add_run(f'(位置: {issue["location"]})')

                    suggestion = doc.add_paragraph(style='List Bullet 2')
                    suggestion.add_run(f'建议: {issue["suggestion"]}')

        # 3. Top recommendations
        doc.add_heading('3. 关键修改项 (Top 10)', 1)
        ...
        for i, issue in enumerate(sorted_issues[:10]):
            doc.add_paragraph(f'{i+1}. {issue["description"]} ({issue["location"]})', style='List Bullet')

        # 4. Positive feedback
        doc.add_heading('4. 鼓励与肯定', 1)

        if self.positive_feedback:
            for feedback in self.positive_feedback:
                doc.add_paragraph(f'• {feedback}', style='List Bullet')
        else:
            doc.add_paragraph('论文整体框架完整，符合学术论文基本要求。')
...
def main():
    ...
    output_path = os.path.join(output_dir, f"thesis_review_{timestamp}.docx")
