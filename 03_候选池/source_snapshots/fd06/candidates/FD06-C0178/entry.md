---
name: ntucool
description: NTU COOL (cool.ntu.edu.tw) — National Taiwan University's Canvas LMS. Use when working with NTU course materials, assignments, grades, announcements, or PDFs. Provides the `cool` CLI and its `cool mcp` MCP server. NTU-only (ADFS SAML auth).
---
...
- `cool` resolves to a path and `whoami` returns user info → setup OK, proceed.
...
- Run the install step yourself (a single `curl … | sh`, see `commands/setup.md`), then have the user type `! cool login` (interactive — Bash tool can't drive the password prompt).
...
- **Gated by `write_level`.** A single ordinal — `none` (default, refuses every submit) / `safe` (only risk-free submissions) / `guarded` (`i_understand` clears soft risks) / `unguarded` (no checks). Resolved from env `NTUCOOL_WRITE_LEVEL`, else the nearest `.ntucool.json` walking parent dirs from cwd, else `none`. **Project-scoped — there is no `~/.ntucool.json` global tier**; the walk-up only crosses `$HOME` incidentally and only when cwd is under it, so don't rely on that. When a submit is refused the error states how to raise the level — relay that to the user; don't change `write_level` yourself.
...
Provide either `files` (paths on the machine running the MCP server → `online_upload`) or `text` (→ `online_text_entry`), never both. Per the non-goals: don't generate the answer content yourself — submit only what the user gives you.
